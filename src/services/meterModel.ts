...
import { syncSession, stopSession } from "./api";
...
type Context = Pick<
	HandlerContext,
	"nowISO" | "sendCall" | "getActiveConnectorId" | "getTransactionId" | "getMeterValuesMeasurands" | "getSoCMode"
>;
...
export function createMeterModel(cpId: string, ctx: Context, cfgPartial?: Partial<MeterConfig>): MeterModel {
	...
	async function emitIfNeededForSession(session: { txId: number; st: MeterState }, connectorId: number) {
		try {
			...
			if (meas.includes("Power.Active.Import"))
				push("Power.Active.Import", "W", Math.max(0, Number((st.powerKW * 1000).toFixed(0))));
			...
			if (meas.includes("SoC")) {
				...
				const location = mode === "outlet" ? "Outlet" : mode === "evse" ? "EVSE" : mode === "none" ? undefined : "EV";
				const socSample: any = {
	...
	const api: MeterModel = {
		start: (tx, conn, initialWh, initialSocPct) => {
			...
			syncSession(cpId, {
				connectorId,
				transactionId: tx,
				meterStartWh: initialWh || 0,
				currentEnergyWh: st.energyWh,
				currentSoc: st.socPct,
				powerKw: st.powerKW,
				currentA: st.currentA,
				voltageV: st.voltageV,
			}).catch(() => {});
		},
		stop: (txMaybe) => {
			for (const [connId, session] of activeSessions) {
				...
				stopSession(cpId, session.txId, session.st.energyWh).catch(() => {});
		...
		tick: async () => {
			...
			for (const [connId, session] of activeSessions) {
				...
				// Sync real-time progress to SQLite
				syncSession(cpId, {
					connectorId: connId,
					transactionId: session.txId,
					currentEnergyWh: session.st.energyWh,
					currentSoc: session.st.socPct,
					powerKw: session.st.powerKW,
					currentA: session.st.currentA,
					voltageV: session.st.voltageV,
				}).catch(() => {});
