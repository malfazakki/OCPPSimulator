...
export const DEFAULT_DEVICE_SETTINGS: DeviceSettings = {
	deviceName: "Simulator-malfazakki",
	model: "EVSE-Sim v1",
	acdc: "AC",
	connectors: 1,
	maxPowerKw: 22,
...
export function normalizeDeviceSettings(partial?: Partial<DeviceSettings>): DeviceSettings {
...
export function normalizeOcppConfiguration(partial?: Partial<OcppConfiguration>): OcppConfiguration {
	...
	const availabilityList = Array.isArray(partial?.Availability) ? partial?.Availability : merged.Availability;

	let sampledData = String(merged.MeterValuesSampledData || DEFAULT_OCPP_CONFIGURATION.MeterValuesSampledData);
	const requiredMeasurands = [
	...
	const existingMeasurands = sampledData
		.split(",")
		.map((s) => s.trim())
		.filter(Boolean);
	...
	const normalized: OcppConfiguration = {
		...
		MeterValueSampleInterval: toNumber(
			merged.MeterValueSampleInterval,
			DEFAULT_OCPP_CONFIGURATION.MeterValueSampleInterval,
		),
		ClockAlignedDataInterval: toNumber(
			merged.ClockAlignedDataInterval,
			DEFAULT_OCPP_CONFIGURATION.ClockAlignedDataInterval,
		),
		...
		NumberOfConnectors: 1,
		TransactionMessageAttempts: toNumber(
			merged.TransactionMessageAttempts,
			DEFAULT_OCPP_CONFIGURATION.TransactionMessageAttempts,
		),
		TransactionMessageRetryInterval: toNumber(
			merged.TransactionMessageRetryInterval,
			DEFAULT_OCPP_CONFIGURATION.TransactionMessageRetryInterval,
		),
		...
		GetConfigurationMaxKeys: toNumber(
			merged.GetConfigurationMaxKeys,
			DEFAULT_OCPP_CONFIGURATION.GetConfigurationMaxKeys,
		),
