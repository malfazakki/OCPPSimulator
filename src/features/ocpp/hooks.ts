...
import { setAutoConnect, setStatus, type ChargePoint, type ConnectionConfig } from "./ocppSlice";
...
export function useOcppConnection(cp?: ChargePoint) {
			...
			try {
				await connectWs(
					...
					() => {
						...
						dispatch(setAutoConnect({ id: cp.id, autoConnect: true }));
					},
					() => {
	...
	const disconnect = useMutation({
		mutationFn: async () => {
			...
			disconnectWs(cp.id);
			dispatch(setStatus({ id: cp.id, status: "disconnected" }));
			dispatch(setAutoConnect({ id: cp.id, autoConnect: false }));
			connect.reset();
		},
	});
