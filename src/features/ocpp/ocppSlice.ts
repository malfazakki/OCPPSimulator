	...
	reducers: {
		addChargePoint: {
			prepare(partial?: Partial<ChargePoint>) {
				...
				const defaultOcppConfig: OcppConfiguration = normalizeOcppConfiguration(partial?.chargePointConfig?.ocppConfig);
		...
		updateConfig(state, action: PayloadAction<{ id: string; patch: Partial<ConnectionConfig> }>) {
			const cp = state.items[action.payload.id];
		...
		setAutoConnect(state, action: PayloadAction<{ id: string; autoConnect: boolean }>) {
			const cp = state.items[action.payload.id];
			if (cp) cp.autoConnect = action.payload.autoConnect;
		},
		...
		updateConnectorStatus(
			state,
			action: PayloadAction<{ id: string; connectorId: number; status: Connector["status"] }>,
		) {
			const cp = state.items[action.payload.id];
			if (cp && cp.runtime?.connectors) {
				const connector = cp.runtime.connectors.find((c) => c.id === action.payload.connectorId);
