...
export function useChargingStatus(cpId: string) {
	...
	const { chargingData, isCharging } = useMemo(() => {
		...
		for (let connectorId = 1; connectorId <= numConnectors; connectorId++) {
			...
			if (call) {
				const payload = call.raw?.[3] as
					| { connectorId?: number; transactionId?: number; meterValue?: MeterValue[] }
					| undefined;
				if (payload) {
		...
		// Determine isCharging: if any connector has an active transaction or is Charging
		const charging = cp?.runtime?.connectors?.some((c) => c.transactionId != null || c.status === "Charging") ?? false;

		return { chargingData: data, isCharging: charging };
	}, [framesQuery.data, numConnectors, cp?.runtime?.connectors]);
