		...
		return (
			<DashboardLayout>
				<div className='p-4 sm:p-6'>
					<div className='text-base sm:text-lg font-semibold'>Charge point not found</div>
					<div className='mt-2 text-xs sm:text-sm text-muted-foreground break-words'>
						The ID "{id}" was not found in your local state. Go back to dashboard and select a charge point.
					</div>
					<div className='mt-4'>
						<Button size='sm' onClick={() => (window.location.hash = "#/")} className='w-full sm:w-auto'>
							Back to Dashboard
						</Button>
					</div>
				</div>
	...
	return (
		<>
			...
			<DashboardLayout>
				<div className='grid gap-4'>
					<div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
						<div className='flex-1 min-w-0'>
							<div className='text-base sm:text-lg font-semibold truncate'>{cp.label}</div>
							<div className='mt-1 text-xs text-muted-foreground break-words'>
								<span className='hidden sm:inline'>CSMS: {cp.config.csmsUrl} • </span>
						...
						<div className='flex items-center gap-2 sm:shrink-0'>
							<div className='hidden sm:flex items-center gap-2'>
								<Button size='sm' variant='outline' onClick={() => setConfigOpen(true)} className='text-xs sm:text-sm'>
									Basic Config
								</Button>
								<Button
									size='sm'
							...
							<div className='flex items-center gap-2 sm:border-l sm:pl-2'>
								<Button
									...
									className='text-xs sm:text-sm'
								>
									{cp.status === "connecting" || (connect.isPending && cp.status !== "connected")
										? "Connecting..."
										: "Connect"}
								</Button>
								<Button
							...
							<div className='flex sm:hidden items-center gap-2'>
								<Button size='sm' variant='outline' onClick={() => setConfigOpen(true)} className='text-xs'>
									Basic
								</Button>
								<Button size='sm' variant='outline' onClick={() => setAdvancedConfigOpen(true)} className='text-xs'>
									Advanced
								</Button>
								<Button size='sm' variant='destructive' onClick={() => setDeleteOpen(true)} className='text-xs'>
									Delete
								</Button>
							</div>
						</div>
				...
				<ChargePointConfigSheet chargePoint={cp} open={configOpen} onOpenChange={(next) => setConfigOpen(next)} />
				...
				<Dialog open={deleteOpen} onOpenChange={(next) => setDeleteOpen(next)}>
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>
								This will remove "{cp.label}" and disconnect the simulated charge point.
							</DialogDescription>
						</DialogHeader>
						<DialogFooter>
							<DialogClose asChild>
